...
Сума вимірів пульсу в трьох спробах (до навантаження, відразу після і після короткого відпочинку)
в ідеалі має бути не більше 200 ударів на хвилину. 
Ми пропонуємо дітям вимірювати свій пульс протягом 15 секунд, 
і приводимо результат до ударів на хвилину множенням на 4:
    S = 4 * (P1 + P2 + P3)
...
відм.   6.4 і менше     4.9 і менше        3.4 і менше         1.9 і менше         0.4 і менше
доб.    6.5 - 11.9      5 - 10.4           3.5 - 8.9           2 - 7.4             0.5 - 5.9
зад.    12 - 16.9       10.5 - 15.4        9 - 13.9            7.5 - 12.4          6 - 10.9
слаб.   17 - 20.9       15.5 - 19.4        14 - 17.9           12.5 - 16.4         11 - 14.9
незад.  21 і більше     19.5 і більше      18 і більше         16.5 і більше       15 і більше

для всіх вікових груп результат "незад." відстоїть від "слабого" на 4, 
той від "задовільного" на 5, а "добрий" від "зад." - на 5.5

тому напишемо функцію ruffier_result(r_index, level), яка буде отримувати
розрахований індекс Руф’є і рівень "незад." для віку тестованого, і віддавати результат
...
def neud_level(age):
    ...
    тут підбираємо рівень "незад." тільки всередині таблиці:
    у віці 7 років "незад." - це індекс 21, далі кожні 2 роки він знижується на 1.5 до значення 15 у 15-16 років '''
...
def ruffier_result(r_index, level):
    ...
    if r_index >= level:
        return 0
    level = level - 4 # це не буде виконуватися, якщо ми вже повернули відповідь "незад."
    if r_index >= level:
        return 1
    level = level - 5 # аналогічно, потрапляємо сюди, якщо рівень як мінімум "зад."
    if r_index >= level:
        return 2
    level = level - 5.5 # наступний рівень
    if r_index >= level:
        return 3
    return 4 # тут опинилися, якщо індекс менший за всі проміжні рівні, тобто тестований чудовий.

def test(P1, P2, P3, age):
    ''' цю функцію можна використовувати зовні модуля для підрахунку індексу Руф’є.
    ...
    if age < 7:
        ...
        result = txt_res[ruffier_result(ruff_index, neud_level(age))] # інтерпретація, переклад числового рівня підготовки в текстові дані
